...
- **禁止行为**：严禁使用 `edit_file` 或 `multi_edit_file` 直接修改现有的 `.tcl`, `.def`, `.pui` 等业务代码文件（例如 `new_post.tcl`）。
- **强制工作流（必须按顺序执行）**：
  1. **复制新建**：当用户要求修改逻辑或修复 Bug 时，必须先使用 `shell_exec` 复制出一个全新的版本。例如：`copy new_post.tcl new_post_v6.tcl`。
  2. **在新文件上修改**：所有的代码编辑、逻辑修复，必须且只能在这个**新生成的文件**（如 v6）上进行。
...
**🏗️ 技术架构与 TCL 核心处理逻辑**
- **无损注入与防覆盖 (Hooking/AOP)**：为防止 UG 后处理构造器覆盖自定义代码，必须使用 TCL 的 `rename` 机制（如 `rename MOM_start_of_path MOM_start_of_path_orig`）挂载拦截器，在原生事件前后动态接管控制权，绝不直接修改 `.pui` 生成的默认事件块。
...
- **终极兜底清理与突破死锁 (Garbage Collection)**：
  ...
  - **流氓文件拦截**：在 `MOM_end_of_program` 阶段，必须扫描根目录，对不在白名单中的 `.nc` 和 `.tmp` 文件执行“格杀勿论”的清理。
  - **突破文件占用锁**：若 NX 主进程死锁导致 `file delete` 失败，需引入后台静默进程强杀方案（例如 `exec cmd.exe /c ping 127.0.0.1 -n 3 > nul & del /f /q ...`）延迟强制删除垃圾文件。
- **路径规范化 (Path Normalization)**：MOM 命令对路径极其敏感，必须强制使用 `string map {"\\" "/"}` 将所有反斜杠替换为正斜杠，消除路径解析错误。
- **文件结构**：核心逻辑集中在 `new_post.tcl`（或其最新版本），配合 `.def` 和 `.pui` 运行。
